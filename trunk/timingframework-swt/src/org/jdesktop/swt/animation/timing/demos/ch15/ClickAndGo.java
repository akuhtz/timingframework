...
public final class ClickAndGo extends Canvas {
	...
	public static void setupGUI(Shell shell, TimingSource ts) {
		f_ball.image = DemoResources.getImage(DemoResources.BLUE_SPHERE,
				shell.getDisplay());
	...
	public ClickAndGo(Composite parent, int style) {
		...
		addPaintListener(new PaintListener() {
			...
			public void paintControl(PaintEvent e) {
				final GC gc = e.gc;
				gc.setBackground(e.widget.getDisplay().getSystemColor(
						SWT.COLOR_WHITE));
				gc.fillRectangle(0, 0, getBounds().width, getBounds().height);

				gc.drawString(
						"Click on the screen an the ball will move to that point in 2 seconds.",
						5, 5);
				gc.drawString(
						"Feel free to change the ball's destination at any time.",
						5, 20);

				gc.setBackground(f_ball.getRecColor());
				gc.setForeground(e.widget.getDisplay().getSystemColor(
						SWT.COLOR_BLACK));
		...
		addMouseListener(new MouseAdapter() {
			...
			public void mouseDown(MouseEvent e) {
				...
				f_ball.animator = new AnimatorBuilder().setDuration(2,
						TimeUnit.SECONDS).build();

				final Point clickPoint = new Point(e.x, e.y);
				final KeyFrames<Point> keyFramesPoint = KeyFrames
						.build(KeyValues.build(clickPoint));
				final PropertySetter toClickPoint = new PropertySetter(f_ball,
						"location", keyFramesPoint);
				f_ball.animator.addTarget(toClickPoint);

				final int rectSize = f_ball.image.getBounds().width;
				final Rectangle clickRect = new Rectangle(e.x, e.y, rectSize
						* (f_die.nextInt(4) + 1), rectSize
						* (f_die.nextInt(4) + 1));
				final KeyFrames<Rectangle> keyFramesRect = KeyFrames
						.build(KeyValues.build(clickRect));
				final PropertySetter toClickPointRect = new PropertySetter(
						f_ball, "rect", keyFramesRect);
				f_ball.animator.addTarget(toClickPointRect);

				final RGB rectRGB = new RGB(f_die.nextInt(255),
						f_die.nextInt(255), f_die.nextInt(255));
				final PropertySetter toClickPointRGB = new PropertySetter(
						f_ball, "rectRGB", rectRGB);
				f_ball.animator.addTarget(toClickPointRGB);
