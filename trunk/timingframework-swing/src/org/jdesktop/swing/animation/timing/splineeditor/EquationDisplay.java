...
public class EquationDisplay extends JComponent implements
	...
	public EquationDisplay(double originX, double originY, double minX,
			double maxX, double minY, double maxY, double majorX, int minorX,
		...
		if (originX < minX || originX > maxX) {
			throw new IllegalArgumentException(
					"originX must be between minX and maxX");
		}
		...
		if (originY < minY || originY > maxY) {
			throw new IllegalArgumentException(
					"originY must be between minY and maxY");
		}
	...
	protected double yPixelToPosition(double pixel) {
		...
		return (getHeight() - pixel - axisH) * (maxY - minY)
				/ (double) getHeight();
	}
	...
	private void drawEquation(Graphics2D g2, AbstractEquation equation) {
		float x = 0.0f;
		float y = (float) yPositionToPixel(equation
				.compute(xPixelToPosition(0.0)));
	...
	private void drawVerticalLabels(Graphics2D g2) {
		...
		for (double x = originX + majorX; x < maxX + majorX; x += majorX) {
			...
			g2.drawString(formatter.format(x), position,
	...
	private void drawAxis(Graphics2D g2) {
		...
		FontMetrics metrics = g2.getFontMetrics();
		g2.drawString(formatter.format(0.0), (int) axisV + 5, (int) axisH
				+ metrics.getHeight());
