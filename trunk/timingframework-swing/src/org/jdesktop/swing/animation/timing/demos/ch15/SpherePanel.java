...
public class SpherePanel extends JPanel {
	...
	SpherePanel(String resourceName, String label) {
		try {
			f_sphereImage = ImageIO.read(DemoResources
					.getResource(resourceName));
		} catch (Exception e) {
			throw new IllegalStateException("Problem loading image "
					+ resourceName, e);
		}
		setPreferredSize(new Dimension(f_sphereImage.getWidth() + 2 * PADDING,
				PANEL_HEIGHT));
		final PropertySetter ps = new PropertySetter(this, "sphereY", 20,
				(PANEL_HEIGHT - f_sphereImage.getHeight()), 20);
