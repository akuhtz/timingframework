...
public class SplineDisplay extends EquationDisplay {
	...
	public void setControl1(Point2D control1) {
		support.firePropertyChange("control1", (Point2D) this.control1.clone(),
	...
	public void setControl2(Point2D control2) {
		support.firePropertyChange("control2", (Point2D) this.control2.clone(),
	...
	private Ellipse2D getDraggableArea(Point2D control) {
		Ellipse2D outer = new Ellipse2D.Double(xPositionToPixel(control.getX())
				- CONTROL_POINT_SIZE / 2.0, yPositionToPixel(control.getY())
				- CONTROL_POINT_SIZE / 2.0, CONTROL_POINT_SIZE,
				CONTROL_POINT_SIZE);
		return outer;
	}

	private void paintSpline(Graphics2D g2) {
		CubicCurve2D spline = new CubicCurve2D.Double(xPositionToPixel(0.0),
				yPositionToPixel(0.0), xPositionToPixel(control1.getX()),
				yPositionToPixel(control1.getY()),
				xPositionToPixel(control2.getX()),
				yPositionToPixel(control2.getY()), xPositionToPixel(1.0),
				yPositionToPixel(1.0));
	...
	private class ControlPointsHandler extends MouseMotionAdapter {
		...
		public void mouseDragged(MouseEvent e) {
			...
			support.firePropertyChange("control"
					+ (selected == control1 ? "1" : "2"), selectedCopy,
					(Point2D) selected.clone());
