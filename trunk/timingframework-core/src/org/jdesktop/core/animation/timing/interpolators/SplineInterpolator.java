...
public final class SplineInterpolator implements Interpolator {
	 ...
	 * defined by (x1,y1) and (x2,y2). The anchor points are implicitly defined
	 * as (0,0) and (1,1).
	 * 
	 * @param x1
	...
	public SplineInterpolator(double x1, double y1, double x2, double y2) {
		if (x1 < 0 || x1 > 1 || y1 < 0 || y1 > 1.0f || x2 < 0 || x2 > 1
				|| y2 < 0 || y2 > 1) {
			throw new IllegalArgumentException(
					"Control points must be in the range [0,1].");
		}
		...
		/*
		 * Now construct the array of all lengths to t in [0,1] at 0.01
		 * increments.
		 */
		...
		/*
		 * Now calculate the fractions so that we can access the lengths array
		 * with values in [0,1]. cumulativeLength now holds the total length of
		 * the spline.
		 */
		final ArrayList<LengthItem> resultLengths = new ArrayList<LengthItem>();
		for (LengthItemBase length : baseLengths) {
			resultLengths.add(new LengthItem(length.getLength(), length.getT(),
					cumulativeLength));
		}
	 ...
	 * <p>
	 * We know that (x0,y0) == (0,0) and (x1,y1) == (1,1) for our splines, so
	 * this simplifies to
	 * <p>
	...
	private Point2D getXY(double t) {
		...
		final Point2D xy = new Point2D((b1 * f_x1) + (b2 * f_x2) + b3,
				(b1 * f_y1) + (b2 * f_y2) + b3);
		return xy;
	}
	 ...
	 * the Y values. See {@link #getXY} for the details.
	 */
	...
	public double interpolate(double fraction) {
		...
		final double proportion = (fraction - prevFraction)
				/ (item.getFraction() - prevFraction);
		final double interpolatedT = prevT
				+ (proportion * (item.getT() - prevT));
	 ...
	 * stores the "length" (which can be thought of as the time elapsed along
	 * the spline path), the <i>t</i> value at this length (used to calculate
	 * the (x,y) point along the spline), and the <i>fraction</i> which is equal
	 * to the length divided by the total absolute length of the spline.
	 * <p>
	 * After we calculate all LengthItems for a given spline, we have a list of
	 * entries which can return the <i>t</i> values for fractional lengths from
	 * 0 to 1.
	 */
	@Immutable
