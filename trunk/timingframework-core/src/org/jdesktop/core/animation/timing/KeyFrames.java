...
public class KeyFrames<T> {
   ...
   * keyTimes based on even division of 0-1 range based on number of keyValues.
  ...
  /**
   * This variant of building a KeyFrames object takes both keyValues (values at
   * each point in time) and keyTimes (times at which values are sampled).
   * 
   * @param keyValues
   *          values that the animation will assume at each of the corresponding
   *          times in keyTimes
   * @param keyTimes
   *          times at which the animation will assume the corresponding values
   *          in keyValues
   * @throws IllegalArgumentException
   *           keyTimes and keySizes must have the same number of elements since
   *           these structures are meant to have corresponding entries; an
   *           exception is thrown otherwise.
   */
  public static <T> KeyFrames<T> build(KeyValues<T> keyValues, KeyTimes keyTimes) {
    return new KeyFrames<T>(keyValues, keyTimes, (Interpolator) null);
  }

  /**
   * Constructs a KeyFrames object using the provided key* structures which will
   * be used to calculate between all times in the keyTimes list. A {@code null}
   * interpolators parameter is equivalent to calling
   ...
   *          in keyValues
   * @param interpolators
   *          collection of Interpolators that control the calculation of values
   *          in each of the intervals defined by keyFrames. If this value is
   *          null, a {@link LinearInterpolator} will be used for all intervals.
   *          If there is only one interpolator, that interpolator will be used
   *          for all intervals. Otherwise, there must be a number of
   *          interpolators equal to the number of intervals (which is one less
   *          than the number of keyTimes).
   * @throws IllegalArgumentException
   *           keyTimes and keyValues must have the same number of elements
   *           since these structures are meant to have corresponding entries;
   *           an exception is thrown otherwise.
   * @throws IllegalArgumentException
   *           The number of interpolators must either be zero (interpolators ==
   *           null), one, or one less than the size of keyTimes.
   */
  public static <T> KeyFrames<T> build(KeyValues<T> keyValues, KeyTimes keyTimes, Interpolator... interpolators) {
   ...
   *           null), one, or one less than the size of keyTimes.
   */
  public static <T> KeyFrames<T> build(KeyValues<T> keyValues, Interpolator... interpolators) {
   ...
   */
  private KeyFrames(KeyValues<T> keyValues, KeyTimes keyTimes, Interpolator... interpolators) {
    ...
    if (interpolators != null && (interpolators.length != (numFrames - 1)) && (interpolators.length != 1)) {
      throw new IllegalArgumentException("interpolators must be either null (implying interpolation for all intervals), "
          + "a single interpolator (which will be used for all intervals), or a number of interpolators equal to "
