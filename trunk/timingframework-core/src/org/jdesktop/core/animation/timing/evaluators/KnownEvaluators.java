...
public final class KnownEvaluators {
	...
	private KnownEvaluators() {
		...
		/*
		 * Add known non-core immutable implementations if we can find them. If
		 * not, that just means we are missing that code (e.g., Swing or SWT).
		 * We do not want to report the missing code as a problem.
		 */
		for (final String className : f_nonCoreImmutable) {
			try {
				/*
				 * The below is a bit messy. For these types we know nothing
				 * about the static typing so we cast to Object. We check the
				 * types dynamically in the createFor method below so this is
				 * not a problem.
				 */
				@SuppressWarnings("unchecked")
				final Class<? extends Evaluator<Object>> evaluatorType = (Class<? extends Evaluator<Object>>) Class
						.forName(className);
	...
	/**
	 * Gets the evaluator for the passed type. This class only registers
	 * immutable evaluator implementations so multiple requests for the same
	 * type will share the same evaluator.
	 * 
	 * @param <T>
	...
	public <T> Evaluator<T> getEvaluatorFor(Class<T> type) {
		...
		for (Evaluator<?> e : f_immutableImplementations) {
			if (e.getEvaluatorClass().equals(type)) {
				/*
				 * Exact type match, cast to the correct (static) type and
				 * return.
				 */
				@SuppressWarnings("unchecked")
				...
				if (e.getEvaluatorClass().isAssignableFrom(type)) {
					/*
					 * Assignable type match, cast to the correct (static) type
					 * and store. We'll continue to look for an exact type match
					 * and only return this result if none is found.
					 */
					@SuppressWarnings("unchecked")
	 ...
	 */
	private <T> Evaluator<T> construct(
			Class<? extends Evaluator<T>> evaluatorType) {
		...
		String msg = "Unable to construct an instance of " + evaluatorType
				+ ", does this class have a no-argument constructor?";
		if (cause == null)
