 ...
 * Animator anim = new AnimatorBuilder().setDuration(1, TimeUnit.SECONDS)
 * 		.addTarget(ps).build();
 * anim.start();
 * </pre>
 * 
...
public class PropertySetter extends TimingTargetAdapter {
	...
	/**
	 * Creates a PropertySetter where the values the property takes on during
	 * the animation are specified in a {@link KeyFrames} object.
	 * 
	 * @param object
	 *            the object whose property will be animated.
	 * @param propertyName
	 *            the name of the property to be animated. For any property name
	 *            {@code "foo"} there must be an accessible {@code setFoo}
	 *            method on the object. If only one value is supplied in
	 *            {@code keyFrames}, the animation will also need a
	 *            {@code getFoo} method.
	 * @param keyFrames
	...
	public PropertySetter(Object object, String propertyName,
		...
		try {
			...
			throw new IllegalArgumentException(
					"Could not find set/get methods for the property \""
							+ propertyName + "\"", e);
		}
	}
	...
	/**
	 * Creates a PropertySetter where the values the property takes on during
	 * the animation are specified as parameters.
	 * 
	 * @param object
	 *            the object whose property will be animated.
	 * @param propertyName
	 ...
	 *            Internally, a {@link KeyFrames} object will be constructed
	 *            that will use times that split the animation duration evenly.
	 *            Supplying only one value implies that this is a <i>to</i>
	 *            animation whose initial value will be determined dynamically
	 *            by a call to the property's get method when the animation
	 *            starts.
	 * @throws IllegalArgumentException
	 *             if the appropriate set/get methods cannot be found for the
	 *             property.
	 ...
	 *            {@code getFoo} method
	 * @param evaluator
	 *            {@link KeyValues} knows how to calculate intermediate values
	 *            for many built-in types, but if you want to supply values in a
	 *            type not known by {@link KeyValues} (which uses
	 *            {@link KnownEvaluators} to find an {@link Evaluator}
	 *            implementation), you will need to supply your own
	 *            {@link Evaluator} implementation.
	 * @param params
	 *            the values that the object will take on during the animation.
	 *            Internally, a {@link KeyFrames} object will be constructed
	 *            that will use times that split the animation duration evenly.
	...
	public <T> PropertySetter(Object object, String propertyName,
			Evaluator<T> evaluator, T... params) {
		this(object, propertyName, KeyFrames.build(KeyValues.build(evaluator,
				params)));
	}

	/**
	 * Translates the property name into the appropriate {@link Method} in the
	 * object to be modified ({@link #f_object}). This method requires the use
	 * of standard JavaBean naming conventions (e.g.,
	 * <tt>propertyName<tt> would become
	 * <tt>setPropertyName</tt>).
	 * 
	 * @throws NoSuchMethodException
	 *             if there is no set/get method on the object with the
	 *             appropriate name.
	...
	/**
	 * This method is sets an initial value for the animation if appropriate;
	 * this accounts for "to" animations, which need to start from the current
	 * value.
	 * <p>
	 ...
	 * {@code propertyName} passed to the constructor) with the appropriate
	 * value of the property given the range of values in the {@link KeyValues}
	 * object and the fraction of the timing cycle that has elapsed.
	 * <p>
	 * This method is not intended for use by application code.
	 */
	@Override
	...
	private void setValue(Object object, Method method, double fraction) {
		try {
			...
			throw new IllegalStateException(
					"An unexpected exception occurred when invoking the method "
							+ method + " on " + object, e);
		}
	}
