	...
	public AccelerationInterpolator(double acceleration, double deceleration) {
		if (acceleration < 0 || acceleration > 1) {
			throw new IllegalArgumentException(String.format(
					"%s value of %.2f lies outside required [0,1] range.",
		...
		if (acceleration > limit) {
			throw new IllegalArgumentException(
					String.format(
							"%s value of %.2f is greater than 1 - %.2f (the %s) = %.2f.",
	...
	public double interpolate(double fraction) {
		if (f_acceleration != 0 || f_deceleration != 0) {
			...
			if (fraction < f_acceleration) {
				 ...
				 * Accelerating
				 */
				double averageRunRate = runRate * (fraction / f_acceleration)
						/ 2.0;
				...
				fraction = runRate
						* (1.0 - (f_acceleration / 2.0) - f_deceleration + tdec
								* (2.0 - pdec) / 2.0);
			} else {
				/*
			...
			/*
			 * Clamp fraction to [0,1] since above calculations may cause
			 * rounding errors.
			 */
			if (fraction < 0) {
				fraction = 0;
