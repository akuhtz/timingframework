...
public final class Viewport {
	...
	public interface MovementRateSource {
		 ...
		 * 
		 * @return the rate of animated viewport movement in nanoseconds per
		 *         pixel.
		 */
	...
	/**
	 * An interpolator to provide a slight acceleration when animated movement
	 * is started.
	 */
	...
	/**
	 * The source to obtain the rate of animated viewport movement in
	...
	public void animateHorizontalMovement(Go toward) {
		...
		if (!(toward == Go.LEFT || toward == Go.RIGHT)) {
			throw new IllegalArgumentException(
					String.format(
							"Animated horizontal viewport movement not allowed toward %s.",
							toward));
		}
		if (f_width < 1) {
		...
		if (pixelsToMove > 0) {
			f_moveHorizontalAnimator = new AnimatorBuilder()
					.setDuration(
							f_rate.getMovementRateNanosPerPixel()
									* pixelsToMove, TimeUnit.NANOSECONDS)
					.setInterpolator(f_moveI)
					.addTarget(
							new Mover(pixelsToMove, true, f_x,
									toward == Go.RIGHT)).build();
	...
	public void animateVerticalMovement(Go toward) {
		...
		if (!(toward == Go.UP || toward == Go.DOWN)) {
			throw new IllegalArgumentException(
					String.format(
							"Animated vertical viewport movement not allowed toward %s.",
							toward));
		}
		if (f_height < 1) {
		...
		if (pixelsToMove > 0) {
			f_moveVerticalAnimator = new AnimatorBuilder()
					...
					.setInterpolator(f_moveI)
					.addTarget(
							new Mover(pixelsToMove, false, f_y,
									toward == Go.DOWN)).build();
			f_moveVerticalAnimator.start();
		}
	}

	/**
	 * Moves the viewport position relative to its current position. If either
	 * of the two movements will move the viewport outside the bounds of the
	 * image being viewed it is adjusted to stay within the bounds.
	 * 
	 * @param deltaX
	 *            the number of pixels to move the viewport horizontally. A
	 *            negative value moves the viewport to the left. A positive
	 *            value moves the viewport to the right. A value of zero leaves
	 *            the horizontal position of the viewport unchanged.
	 * @param deltaY
	 *            the number of pixels to move the viewport vertically. A
	 *            negative value moves the viewport up. A positive value moves
	 *            the viewport down. A value of zero leaves the vertical
	 ...
	 * 
	 * @param value
	 *            the rate of animated viewport movement in nanoseconds per
	 *            pixel or value less than one to reset the movement rate to its
	 *            default value.
	 */
	...
	/**
	 * Disposes this viewport by ensuring that any running animations are
	 * stopped.
	 */
	public void dispose() {
		if (f_moveHorizontalAnimator != null)
