 ...
 * <p>
 * Active rendering is used if the <tt>org.jdesktop.renderer.active</tt> property is defined. For example with the
 * following passed to the Java interpreter.
 * 
 * <pre>
...
public final class JRendererFactory {
     ...
     * </pre>
     * 
     * This method constructs and starts an {@link SwingTimerTimingSource} or a {@link ManualTimingSource} and sets it
     * as the default for all animations. A handle to this timing source can be obtained by invoking
     * {@link JRenderer#getTimingSource()}, for example, if it needs to be disposed.
     * 
     * @param on
     ...
     * @param hasChildren
     *            if <tt>on</tt> manages child Swing components. {@code true} if it does. {@code false} if it does not.
     *            This helps the renderer improve performance if no child components are ever used.
     * @return a renderer for <tt>on</tt>.
     */
    public static JRenderer getDefaultRenderer(
        JRendererPanel on, JRendererTarget<GraphicsConfiguration, Graphics2D> target, boolean hasChildren) {
     ...
     * 
     * @return {@code true} if active rendering is desired, {@code false} otherwise.
     */
