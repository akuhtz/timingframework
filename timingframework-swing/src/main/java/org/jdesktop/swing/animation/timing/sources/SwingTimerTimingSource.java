...
/**
 * A timing source based upon a Swing {@link Timer} and the {@link SwingUtilities#invokeLater(Runnable)} method. This
 * implementation ensures that calls to registered {@code TickListener} and {@code PostTickListener} objects are always
 * made within the thread context of the Swing EDT.
 * <p>
 ...
 * </pre>
 * 
 * If you are not sure what period to set, use the {@link #SwingTimerTimingSource()} constructor which uses a reasonable
 * default value of 15 milliseconds.
 * <p>
 * Tasks submitted to {@link #submit(Runnable)} and calls to registered {@code TickListener} and
 * {@code PostTickListener} objects from this timing source are always made in the context of the Swing EDT.
 * 
 * @author Tim Halloran
 */
public final class SwingTimerTimingSource extends TimingSource {
     ...
     * <p>
     * The Swing timer requires a period of at least 1 millisecond. If the period passed is smaller, it is set to 1
     * millisecond. It is also not recommended to pass a period much larger than a day as this could lead to integer
     * overflow in the case that the value in milliseconds cannot be represented as an <tt>int</tt> to be passed to the
     * Swing {@link Timer}.
     * 
     * @param period
