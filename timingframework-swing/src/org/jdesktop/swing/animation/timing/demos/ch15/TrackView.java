...
public class TrackView extends JComponent {
	...
	public TrackView() {
		try {
			car = ImageIO.read(DemoResources
					.getResource(DemoResources.BEETLE_RED));
			track = ImageIO
					.read(DemoResources.getResource(DemoResources.TRACK));
		} catch (Exception e) {
	...
	public void paintComponent(Graphics g) {
		...
		/*
		 * Now draw the car. The translate/rotate/translate settings account for
		 * any nonzero carRotation values.
		 */
		...
		/*
		 * Now the graphics has been set up appropriately; draw the car in
		 * position
		 */
		g2d.drawImage(car, carPosition.x - carWHalf, carPosition.y - carHHalf,
				null);
	}
	...
	public void setCarPosition(Point newPosition) {
		repaint(0, carPosition.x - carWHalf, carPosition.y - carHHalf, carW,
				carH);
	...
	public void setCarRotation(double newDegrees) {
		...
		repaint(0, carPosition.x - carW, carPosition.y - carH, 2 * carW,
				2 * carH);
	}
