...
public final class ClickAndGo extends JPanel {
	...
	private static final Map f_desktopHints = (Map) java.awt.Toolkit
			.getDefaultToolkit().getDesktopProperty("awt.font.desktophints");

	public static void setupGUI() {
		try {
			f_ball.image = ImageIO.read(DemoResources
					.getResource(DemoResources.BLUE_SPHERE));
		} catch (IOException e) {
	...
	public ClickAndGo() {
		...
		addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				final int rectSize = f_ball.image.getWidth();
				final Rectangle clickRect = new Rectangle(e.getX(), e.getY(),
						rectSize * (f_die.nextInt(4) + 1), rectSize
								* (f_die.nextInt(4) + 1));
				...
				final Color rectColor = new Color(f_die.nextInt(255),
						f_die.nextInt(255), f_die.nextInt(255));
				final PropertySetter toClickPointColor = new PropertySetter(
						f_ball, "rectColor", rectColor);
				f_ball.animator.addTarget(toClickPointColor);
	...
	protected void paintComponent(Graphics g) {
		...
		g2d.drawString(
				...
				5, 20);
		g2d.drawString(
				"Feel free to change the ball's destination at any time.", 5,
				35);
