 ...
 * 
 * Animator animator = new AnimatorBuilder().setDuration(1, TimeUnit.SECONDS)
 * 		.addTarget(myTarget).build();
 * animator.start();
 * </pre>
 * 
 ...
 * 		.setDuration(500, TimeUnit.MILLISECONDS).setRepeatCount(4)
 * 		.addTarget(myTarget).build();
 * animator.start();
 * </pre>
 * 
...
public final class Animator implements TickListener {
	 ...
	 * {@link AnimatorBuilder#build()}.
	 */
	@Unique("return")
	Animator(long duration, TimeUnit durationTimeUnit, EndBehavior endBehavior,
			Interpolator interpolator, RepeatBehavior repeatBehavior,
			long repeatCount, Direction startDirection,
	 ...
	 * as soon as it is added. If {@code target} is already on the list of
	 * targets in this animation, it is not added again (there will be only one
	 * instance of any given target in any animation's list of targets).
	 * 
	 * @param target
	 ...
	 * notified of each timing event while the animation is running.
	 * <p>
	 * The set of registered {@link TimingTarget} objects will be empty after
	 * this call returns.
	 */
		...
		synchronized (f_lock) {
			if (f_running)
				throw new IllegalStateException(
						"start() can't be invoked on a running animation.");
	...
	public void startReverse() {
		synchronized (f_lock) {
			...
			f_startTimeNanos = f_cycleStartTimeNanos = System.nanoTime();
			f_listenersToldAboutBegin = f_timeToStop = f_tellListenersAboutRepeat = false;
			f_currentDirection = (f_startDirection == Direction.FORWARD) ? Direction.BACKWARD
					: Direction.FORWARD;
			f_pauseBeginTimeNanos = 0;
			f_running = true;
		}
	...
	public void reverseNow() {
		synchronized (f_lock) {
			if (!f_running)
				throw new IllegalStateException(
						"reverseNow() can only be invoked on a running animation.");
			 ...
			 */
			f_currentDirection = (f_currentDirection == Direction.FORWARD) ? Direction.BACKWARD
					: Direction.FORWARD;
		}
	}
	...
	public void resume() {
		...
		synchronized (f_lock) {
			...
			if (paused) {
				long pauseDeltaNanos = System.nanoTime()
						- f_pauseBeginTimeNanos;
	 ...
	 *            value of current time, from {@link System#nanoTime()}, to use
	 *            in calculating the elapsed time.
	...
	private double calcInterpolatedTimingFraction(long nanoTime) {
		...
		if ((f_duration != INFINITE) && (f_repeatCount != INFINITE)
				&& (currentCycleCount >= f_repeatCount)) {
			/*
			...
			default:
				throw new IllegalStateException(String.format(
						"switch on %s can't handle case %s.",
						EndBehavior.class.getName(), f_endBehavior.toString()));
			}
			f_timeToStop = true;
		} else if ((f_duration != INFINITE)
				&& (cycleElapsedTimeNanos > durationNanos)) {
			/*
			...
			/*
			 * Animation Mid-Stream: Calculate fraction of animation between
			...
			if (f_duration != INFINITE) {
				 ...
				 */
				fraction = (double) cycleElapsedTimeNanos
						/ (double) durationNanos;
				if (f_currentDirection == Direction.BACKWARD) {
					/*
					 * If this is a reversing cycle, want to know inverse
					 * fraction; how much from start to finish, not finish to
					 * start.
					 */
					fraction = 1.0 - fraction;
				}
				/*
				 * Clamp fraction in case timing mechanism caused out of bounds
				 * value.
				 */
		...
		return f_interpolator == null ? fraction : f_interpolator
