...
/**
 * This class interpolates fractional values to animate movement with acceleration at the beginning and deceleration at
 * the end.
 * <p>
    ...
    public AccelerationInterpolator(double acceleration, double deceleration) {
        if (acceleration < 0 || acceleration > 1) {
            throw new IllegalArgumentException(
                String.format("%s value of %.2f lies outside required [0,1] range.", "Acceleration", acceleration));
        }
        if (deceleration < 0 || deceleration > 1) {
            throw new IllegalArgumentException(
                String.format("%s value of %.2f lies outside required [0,1] range.", "Deceleration", deceleration));
        }
        double limit = 1.0 - deceleration;
        if (acceleration > limit) {
            throw new IllegalArgumentException(String
                .format("%s value of %.2f is greater than 1 - %.2f (the %s) = %.2f.", "Acceleration", acceleration,
                    deceleration, "deceleration", limit));
        }
        limit = 1.0 - acceleration;
        if (deceleration > limit) {
            throw new IllegalArgumentException(String
                .format("%s value of %.2f is greater than 1 - %.2f (the %s) = %.2f.", "Deceleration", deceleration,
