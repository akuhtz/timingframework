...
/**
 * Manages the location of a viewport into a larger image. The viewport may be resized, moved directly, or moved with an
 * animation.
 * <p>
 * The size of the image the viewport is into is not allowed to be changed. If the image size changes a new viewport
 * must be constructed.
 * 
 * @author Tim Halloran
 */
public final class Viewport {
    ...
    /**
     * A default source to obtain the rate of animated viewport movement in nanoseconds per pixels.
     */
    ...
    /**
     * An interpolator to provide a slight acceleration when animated movement is started.
     */
    final Interpolator f_moveI = new AccelerationInterpolator(0.2, 0);

    /**
     * The source to obtain the rate of animated viewport movement in nanoseconds per pixel.
     */
    ...
    /**
     * Starts or stops animated horizontal movement of this viewport. The method sets up the animation and returns
     * immediately.
     * 
     * @param toward
     *            the direction of the animation, either {@link Go#LEFT} or {@link Go#RIGHT}. A value of {@link Go#STOP}
     *            stops animated horizontal movement, if any is in progress.
     * @throws IllegalArgumentException
     *             if toward is not one of {@link Go#LEFT}, {@link Go#RIGHT}, or {@link Go#STOP}.
    ...
    public void animateHorizontalMovement(Go toward) {
        ...
        if (!(toward == Go.LEFT || toward == Go.RIGHT)) {
            throw new IllegalArgumentException(
                String.format("Animated horizontal viewport movement not allowed toward %s.", toward));
        }
        if (f_width < 1) {
        ...
        if (pixelsToMove > 0) {
            f_moveHorizontalAnimator =
                new Animator.Builder()
                    .setDuration(f_rate.getMovementRateNanosPerPixel() * pixelsToMove, NANOSECONDS)
                    .setInterpolator(f_moveI).addTarget(new Mover(pixelsToMove, true, f_x, toward == Go.RIGHT)).build();
            f_moveHorizontalAnimator.start();
        }
    }

    /**
     * Starts or stops animated vertical movement of this viewport. The method sets up the animation and returns
     * immediately.
     * 
     * @param toward
     *            the direction of the animation, either {@link Go#UP} or {@link Go#DOWN}. A value of {@link Go#STOP}
     *            stops animated horizontal movement, if any is in progress.
     * @throws IllegalArgumentException
     *             if toward is not one of {@link Go#UP}, {@link Go#DOWN}, or {@link Go#STOP}.
     * @throws IllegalStateException
    ...
    public void animateVerticalMovement(Go toward) {
        ...
        if (!(toward == Go.UP || toward == Go.DOWN)) {
            throw new IllegalArgumentException(
                String.format("Animated vertical viewport movement not allowed toward %s.", toward));
        }
        if (f_height < 1) {
        ...
        if (pixelsToMove > 0) {
            f_moveVerticalAnimator =
                new Animator.Builder()
                    .setDuration(f_rate.getMovementRateNanosPerPixel() * pixelsToMove, NANOSECONDS)
                    .setInterpolator(f_moveI).addTarget(new Mover(pixelsToMove, false, f_y, toward == Go.DOWN)).build();
            f_moveVerticalAnimator.start();
        }
    }

    /**
     * Moves the viewport position relative to its current position. If either of the two movements will move the
     * viewport outside the bounds of the image being viewed it is adjusted to stay within the bounds.
     * 
     * @param deltaX
     *            the number of pixels to move the viewport horizontally. A negative value moves the viewport to the
     *            left. A positive value moves the viewport to the right. A value of zero leaves the horizontal position
     *            of the viewport unchanged.
     * @param deltaY
     *            the number of pixels to move the viewport vertically. A negative value moves the viewport up. A
     *            positive value moves the viewport down. A value of zero leaves the vertical position of the viewport
     *            unchanged.
     * @throws IllegalStateException
    ...
    /**
     * Sets the rate of animated viewport movement. A value less than one resets the movement rate to its default value.
     * 
     * @param nanosPerPixel
     *            the rate of animated viewport movement in nanoseconds per pixel or value less than one to reset the
     *            movement rate to its default value.
     */
    ...
    /**
     * Sets a source for the rate of animated viewport movement. A value of {@code null} resets the movement rate to its
     * default value.
     * 
     * @param source
     *            a source for the rate of animated viewport movement or {@code null} to reset the movement rate to its
     *            default value.
     */
